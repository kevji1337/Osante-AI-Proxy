...
import (
	...
	"github.com/kevji1337/Osante-AI-Proxy/internal/webdav"
)
...
func (w *WebDAVService) UpdateWebDAVConfig(url, username, password string) error {
	webdavConfig := &config.WebDAVConfig{
		...
		Username:   username,
		Password:   password,
		ConfigPath: "/Osante Proxy/config",
		StatsPath:  "/Osante Proxy/stats",
	}
...
func (w *WebDAVService) BackupToWebDAV(filename string) error {
	...
	tempDir := filepath.Join(homeDir, ".Osante", "temp")
