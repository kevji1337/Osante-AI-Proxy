...
// currentRequestEndpoint returns the current endpoint for serving a request,
// skipping endpoints that are in usage-limit cooldown. It advances currentIndex
// past skipped endpoints so subsequent rotations start from a usable one.
// Returns an empty endpoint if every enabled endpoint is in cooldown.
func (p *Proxy) currentRequestEndpoint() config.Endpoint {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	start := p.currentIndex % n
	for i := 0; i < n; i++ {
		idx := (start + i) % n
		if !p.endpointInCooldown(endpoints[idx].Name) {
			p.currentIndex = idx
