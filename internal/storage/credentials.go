...
// MarkCredentialUsageLimit puts a single token-pool credential into cooldown
// until cooldownUntil (e.g. a FreeModel "usage limit reached" reset time). The
// credential is skipped by GetUsableEndpointCredential until the cooldown
// expires, so the pool rotates to the next token without dropping the whole
// endpoint.
func (s *SQLiteStorage) MarkCredentialUsageLimit(id int64, errMsg string, cooldownUntil time.Time, now time.Time) error {
	s.mu.Lock()
	defer s.mu.Unlock()
